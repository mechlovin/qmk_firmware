...
bool ecsm_matrix_scan(matrix_row_t current_matrix[]) {
    ...
    for (int col = 0; col < sizeof(col_channels); col++) {
        for (int row = 0; row < 5; row++) {
            ecsm_sw_value[row][col + 9] = ecsm_readkey_raw(1, row, col);
            updated |= ecsm_update_key(&current_matrix[row], row, col + 9, ecsm_sw_value[row][col + 9]);
        }
    }
    return updated;
}
